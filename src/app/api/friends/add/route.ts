...
export async function POST(req:Request) {
    try {
        ...
        const isAlreadyAdded =  (await fetchRedis('sismember',`user:${idToAdd}:incoming_friend_requets`,session.user.id) )as 0 | 1
      ...
      if (isAlreadyAdded) {
        ...
        return new Response('friends Added', { status: 200 })
    } catch (error) {
